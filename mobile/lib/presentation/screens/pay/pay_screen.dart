...
class _PayScreenState extends State<PayScreen> {
  ...
  void _executePayment(BuildContext context, String serviceName, double amount, String reference, Color color) async {
    final comptesRepo = sl<ComptesRepository>();
    final opsRepo = sl<OperationsRepository>();

    String? compteNum;
    try {
      final comptes = await comptesRepo.getMyComptes();
      if (comptes.isEmpty) {
        // Fallback aux mocks si pas de comptes (cas dégradé).
        final authState = context.read<AuthBloc>().state;
        String currentPhone = '771234567';
        if (authState is AuthSuccess && authState.phone != null) {
          currentPhone = authState.phone!;
        }
        final mocks = MockData.getAccountsForPhone(currentPhone);
        final defaultAccount = mocks.firstWhere(
          (acc) => acc['isDefaultAccount'] == true,
          orElse: () => mocks.first,
        );
        compteNum = defaultAccount['numeroCompte'] as String? ?? defaultAccount['id'] as String?;
      } else {
        compteNum = comptes
            .firstWhere(
              (c) => c.isDefaultAccount,
              orElse: () => comptes.first,
            )
            .numeroCompte;
      }
    } catch (_) {
      // Lecture comptes en échec — on bascule sur mocks.
      final authState = context.read<AuthBloc>().state;
      String currentPhone = '771234567';
      if (authState is AuthSuccess && authState.phone != null) {
        currentPhone = authState.phone!;
      ...
      if (mocks.isNotEmpty) {
        ...
        compteNum = defaultAccount['numeroCompte'] as String?
                 ?? defaultAccount['id'] as String?;
      }
    }

    if (compteNum == null) {
      if (mounted) _showErrorPopup(context, 'Aucun compte disponible.');
      return;
    }

    try {
      await opsRepo.pay(
        fromAccountNum: compteNum,
      ...
      if (mounted) {
        _showErrorPopup(context, e.toString().replaceFirst('Exception: ', ''));
      }
    }
  }
