...
class _LoanDetailScreenState extends State<LoanDetailScreen> {
    ...
    return Scaffold(
      body: BlocListener<LoanBloc, LoanState>(
        listenWhen: (prev, curr) =>
            (curr.installmentPaymentSuccessPulse &&
                !prev.installmentPaymentSuccessPulse) ||
            (curr.status == LoanStatus.failure &&
                curr.errorMessage != null &&
                (prev.status != LoanStatus.failure ||
                    prev.errorMessage != curr.errorMessage)),
        listener: (context, state) {
          if (state.installmentPaymentSuccessPulse) {
            context.read<LoanBloc>().add(const ClearInstallmentPaymentPulse());
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () => context.read<LoanBloc>().add(
                        FetchLoanDetails(widget.loanId),
                      ),
                          ...
                          ClipRRect(
                            ...
                            child: LinearProgressIndicator(
                              ...
                              valueColor: AlwaysStoppedAnimation<Color>(
                                loan.isLate ? Colors.red : Colors.green,
                              ),
                              minHeight: 10,
                            ),
                          ),
                                    ...
                                    Container(
                                      ...
                                      child: Text(
                                        ...
                                        style: TextStyle(
                                          ...
                                          color: _garantieStatutColor(g.statut),
                                        ),
                                      ),
                                    ),
                                  ],
  ...
  Color _garantieStatutColor(String statut) {
    final s = statut.toLowerCase();
    if (s.contains('valid')) return Colors.green;
    if (s.contains('cours') || s.contains('constitution')) return Colors.orange;
    if (s.contains('libér') || s.contains('leve')) return Colors.blueGrey;
