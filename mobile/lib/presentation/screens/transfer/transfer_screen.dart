...
class _TransferScreenState extends State<TransferScreen> {
  ...
  final OperationsRepository _opsRepo = sl<OperationsRepository>();

  String? _fromAccountId;
  String? _toAccountId;
  bool _isLoading = false;

  /// Comptes chargés depuis le backend (avec fallback mock).
  ...
  Future<void> _loadAccounts() async {
    try {
      ...
      setState(() => _accounts = MockData.getAccountsForPhone(currentPhone));
    }
  }
  ...
  void _executeTransfer() async {
    ...
    try {
      // Le backend identifie les comptes par leur `numeroCompte`, pas par l'ID
      // technique côté UI — on remappe via la liste d'accounts.
      final from = _accounts.firstWhere(
        (a) => a['id'] == _fromAccountId,
        orElse: () => <String, dynamic>{},
      );
      final to = _accounts.firstWhere(
        (a) => a['id'] == _toAccountId,
        orElse: () => <String, dynamic>{},
      );
      final fromNum = (from['numeroCompte'] as String?) ?? _fromAccountId!;
      final toNum   = (to['numeroCompte']   as String?) ?? _toAccountId!;

      await _opsRepo.transfer(
        fromAccountNum: fromNum,
        toAccountNum:   toNum,
        montant:        double.parse(_amountController.text),
        libelle:        _reasonController.text,
      );
