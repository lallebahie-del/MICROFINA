...
public class CreditWorkflowController {
    ...
    public ResponseEntity<Map<String, Object>> soumettre(@PathVariable Long idCredit) {
        Credits c = workflowService.soumettreDossier(idCredit);
        return ResponseEntity.ok(creditSummary(c));
    }
    ...
    public ResponseEntity<Map<String, Object>> completude(
    ...
    public ResponseEntity<Map<String, Object>> visaRc(
            @PathVariable Long idCredit,
            @RequestBody(required = false) WorkflowDecisionRequest req) {
    ...
    public ResponseEntity<Map<String, Object>> comiteApprouver(
            @PathVariable Long idCredit,
            @RequestBody(required = false) WorkflowDecisionRequest req) {
    ...
    @PreAuthorize("hasAuthority('PRIV_COMITE_CREDIT') or hasAuthority('ROLE_ADMIN')")
    public ResponseEntity<Map<String, Object>> comiteRejeter(
            @PathVariable Long idCredit,
            @RequestBody(required = false) WorkflowDecisionRequest req) {
    ...
    public ResponseEntity<Map<String, Object>> visaSf(
            @PathVariable Long idCredit,
            @RequestBody(required = false) WorkflowDecisionRequest req) {
    ...
    public ResponseEntity<Map<String, Object>> debloquer(
            @PathVariable Long idCredit,
    ...
    @PreAuthorize("hasAuthority('PRIV_COMITE_CREDIT') or hasAuthority('ROLE_ADMIN')")
    public ResponseEntity<List<Map<String, Object>>> comitePending() {
        return ResponseEntity.ok(
            workflowService.getComitePending().stream()
                .map(this::creditSummary)
                .toList()
        );
    }
    ...
    private Map<String, Object> creditSummary(Credits c) {
        return Map.of(
            "idCredit",      c.getIdCredit(),
            "numCredit",     c.getNumCredit() != null ? c.getNumCredit() : "",
            "statut",        c.getStatut().name(),
            "etapeCourante", c.getEtapeCourante()
        );
    }
}
